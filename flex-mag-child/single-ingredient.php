						...
						<?php global $post; $mvp_post_temp = get_post_meta($post->ID, "mvp_post_template", true); if ( $mvp_post_temp == "temp3" || $mvp_post_temp == "temp5" || $mvp_post_temp == "temp6" ) { } else { ?>
							<header id="post-header">
								<?php if ( ! is_singular( 'scoreboard' )) { ?>
									<span class="post-head-cat" style="color: #fff;"><?php $ing_category = the_terms( $post->ID, 'ing_category'); echo esc_html( $ing_category->cat_name ); ?></span>
								<?php } ?>
											...
											<div class="post-info-in">
												<div class="post-info-right left relative">
													<div class="post-info-name left relative">
														<span class="post-info-text"><?php $terms = get_the_terms( $post->ID , 'goal' ); $count = count($terms); if ( $count >= 2 ){ echo "Goals:"; } else { echo "Goal:"; } ?></span> <span class="author-name vcard fn" itemprop="author"><?php $goal = the_terms( $post->ID, 'goal'); echo esc_html( $goal->cat_name ); ?></span>
													</div><!--post-info-name-->
													<div class="post-info-name left relative" style="border-bottom: 0;">
														<span class="post-info-text"><?php $terms = get_the_terms( $post->ID , 'ing_tag' ); $count = count($terms); if ( $count >= 2 ){ echo "Tags:"; } else { echo "Tag:"; } ?></span> <span class="author-name"><?php $ing_tag = the_terms( $post->ID, 'ing_tag'); echo esc_html( $ing_tag->cat_name ); ?></span>
													</div><!--post-info-date-->
												</div><!--post-info-right-->
											</div><!--post-info-in-->
										</div><!--post-info-out-->
