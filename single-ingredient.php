						...
						<?php global $post; $mvp_post_temp = get_post_meta($post->ID, "mvp_post_template", true); if ( $mvp_post_temp == "temp3" || $mvp_post_temp == "temp5" || $mvp_post_temp == "temp6" ) { } else { ?>
							<header id="post-header">
								<?php if ( ! is_singular( 'scoreboard' )) { ?>
									<a class="post-cat-link" href="<?php $category = get_the_category(); $category_id = get_cat_ID( $category[0]->cat_name ); $category_link = get_category_link( $category_id ); echo esc_url( $category_link ); ?>"><span class="post-head-cat"><?php $category = get_the_terms( $post->ID , 'ing_category'); echo esc_html( $category[0]->cat_name ); ?></span></a>
								<?php } ?>
